...
if(typeof sharedSettings.arcadeMode !== 'boolean'){
  sharedSettings.arcadeMode = DEFAULT_SETTINGS.arcadeMode;
}
...
let addCargo = storedAddCargo === null
  ...
  : storedAddCargo === 'true';
const storedArcadeMode = getStoredItem('settings.arcadeMode');
sharedSettings.arcadeMode = storedArcadeMode === null
  ? DEFAULT_SETTINGS.arcadeMode
  : storedArcadeMode === 'true';
...
function saveSettings(){
  ...
  setStoredItem('settings.arcadeMode', sharedSettings.arcadeMode);
  ...
  console.log('[settings] save', {
    ...
    addCargo,
    arcadeMode: sharedSettings.arcadeMode,
...
function resetSettingsToDefaults(){
  ...
  addsUiState.arcade = DEFAULT_SETTINGS.arcadeMode;
...
const addsUiState = {
  ...
  arcade: sharedSettings.arcadeMode === true
};
...
if(addsArcadeBtn){
  ...
  addFieldAuditListener(addsArcadeBtn, 'click', (event) => {
    ...
    sharedSettings.arcadeMode = addsUiState.arcade;
    saveSettings();
    setTumblerState(addsArcadeBtn, addsUiState.arcade);
    syncArcadeCargoPreview(addsUiState.arcade);
  });
...
function handleSettingsLayerShow(){
  ...
  // Подпись карты перечитывается ЗАНОВО при каждом показе панели.
  //
  // Иначе она остаётся от той минуты, когда страницу загрузили, а к моменту показа
  // настройка уже другая: кнопка «Advanced Settings» первым делом зовёт loadSettings(),
  // и та достаёт из localStorage карту, выбранную в прошлый раз. Картинка с кирпичами
  // читает настройку в момент отрисовки и показывает восстановленную карту, а подпись
  // продолжала называть ту, что была до восстановления. Панель говорила «random», а
  // игра грузила совсем другое поле — и разошлись они молча.
  syncFieldSelectorState();
  syncAccuracyCrackWatcher();
  startPreviewSimulation();
  // И картинка тоже. Обычно её перерисовывают стрелки, но между двумя показами панели
  // карта могла смениться и без них: настройку перечитали из хранилища, а классические
  // правила её и вовсе переписывают своим броском. Без force перерисовка пропускается,
  // когда индекс совпал с прошлым показанным, — и в рамке остаётся поле из прошлого раза.
  updateMapPreviewIndex(sharedSettings.mapIndex, { force: true });
