...
// === 1h. Бумага одна на всю игру, и место под значки держится заранее ===
//
// Окошко покрашено палитрой подсказок инвентаря — той же, что и баннер раунда. Это
// единственная «бумага», которую игра показывает поверх поля, и второй расцветки у неё
// быть не должно: разойдутся — на экране окажутся два разных сорта бумаги.
{
  const токены = /--author-links-background:\s*var\(--inventory-tooltip-background\)/.test(styles);
  assert(токены,
    '1h: окошко ссылок покрашено само по себе, а не палитрой подсказок инвентаря');

  const правило = /#menuLayer #modeMenu \.author-links\s*\{([^}]*)\}/.exec(styles);
  assert(правило, '1i: не найдено правило окошка');
  assert(/var\(--author-links-background/.test(правило[1]),
    '1j: окошко перестало брать цвет из своих токенов — палитра разъедется');

  // Значков ещё нет, но место под них уже занято: приедут файлы — строки не поедут.
  const строки = html.match(/<a class="author-links__row"[\s\S]*?<\/a>/g) || [];
  for(const строка of строки){
    assert(/class="author-links__icon"/.test(строка),
      '1k: у строки ссылок нет места под значок — с приездом картинок раскладка поедет');
  }
}
