...
const { читатьАльфу, ПОРОГ_АЛЬФЫ } = require('./sprite-alpha-utils.js');
...

{
  ...
  const { width: Ш, height: В, alpha } = читатьАльфу(бумага[1]);
  ...
  // Поля глубже обрыва. Считается по самой картинке: сколько от грани холста идёт
  // прозрачного — сперва пустое поле, потом само рваньё. Крайние доли краёв пропущены:
  // в самых углах обрыв уходит вглубь на сотни точек, но текста там не бывает.
  const п = (x, y) => alpha[y * Ш + x];
  const глубина = (вдоль, длина, взять) => {
    const края = [];
    for(let i = 0; i < вдоль; i += 1){
      let k = 0;
      while(k < длина && п(...взять(i, k)) < ПОРОГ_АЛЬФЫ) k += 1;
      края.push(k);
    }
    const срез = края.slice(Math.round(вдоль * 0.05), Math.round(вдоль * 0.95)).sort((a, b) => a - b);
    return срез[Math.floor(срез.length * 0.99)];
  };
  const вТочках = (v, сторона) => v * (сторона === 'бок' ? ШИРИНА / Ш : (ШИРИНА * В / Ш) / В);
  const обрыв = {
    слева: глубина(В, Ш, (y, k) => [k, y]),
    справа: глубина(В, Ш, (y, k) => [Ш - 1 - k, y]),
    сверху: глубина(Ш, В, (x, k) => [x, k]),
    снизу: глубина(Ш, В, (x, k) => [x, В - 1 - k])
  };
  for(const [имя, сырое] of Object.entries(обрыв)){
    const надо = вТочках(сырое, имя === 'слева' || имя === 'справа' ? 'бок' : 'верт');
    const есть = имя === 'сверху' ? поле.верх : имя === 'снизу' ? поле.низ : поле.бок;
    assert(есть >= надо,
