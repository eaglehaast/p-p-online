...
//
// Позже тем же путём прошёл лист со ссылками автора. Он приехал в проект уже после того
// заезда и сразу стал бы самой тяжёлой картинкой в игре — тяжелее фона игрового экрана:
//
//   mm_links_paper            1118 КБ →  135 КБ   −88%
//
// Здесь важна не только экономия: у листа нарисован рваный край, то есть вся картинка
// держится на альфе. Она при этом сжатии сохраняется бит-в-бит — силуэт обрыва не поехал
// ни на одну точку, разошёлся только цвет, на 2.0 из 255.
...
// Лист со ссылками автора стоит особняком: он живёт только в стилях, фоном у окошка, и
// грузиться из кода не должен. Окошко открывают редко, и 135 КБ на старте были бы ровно
// тем, ради чего весь этот заезд и затевался. Формат с него спрашивается тот же, а
// предзагрузка — наоборот, запрещена.
const ТОЛЬКО_В_СТИЛЯХ = [
  'ui_mainmenu/mm_links_paper',
];

const ВСЕ = [...CONVERTED, ...ТОЛЬКО_В_СТИЛЯХ];

// === 1. ГЛАВНОЕ: тяжёлых PNG больше нет ни на диске, ни в ссылках ===
{
  for(const base of ВСЕ){
...
// ссылка останется целой, а фон исчезнет молча.
{
  ...
  assert(total < 1000 * 1024,
    `2e: четырнадцать картинок весят ${Math.round(total / 1024)} КБ вместо примерно 750 — `
...
// Без этого пункт 1 прошёл бы и на варианте «фон просто убрали из стилей».
{
  ...
  // А лист со ссылками — наоборот: он обязан остаться только в стилях. Стоит ему попасть
  // в предзагрузку, и 135 КБ поедут на старте у каждого, кто окошко даже не откроет.
  for(const base of ТОЛЬКО_В_СТИЛЯХ){
    assert(styles.includes(`${base}.webp`), `3f: в стилях пропала ссылка на ${base}.webp`);
    assert(!script.includes(`${base}.webp`),
      `3g: ${base}.webp попал в код — окошко открывают редко, и на старте он лишний`);
  }
}
