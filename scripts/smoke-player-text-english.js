...
// с английскими «No one survived.» и «No one won the round.» на том же экране.
//
...
//
// Три исхода рисуются рядом и одним шрифтом, поэтому русский среди них особенно заметен:
// именно так «Игра окончена. Ничья.» и стояла между двумя английскими строками.
{
  const block = source.slice(source.indexOf('const lines = ["No one survived."'));
  assert(block.length > 0, '2: конец матча не найден в script.js');
  const endText = block.slice(0, block.indexOf('shouldDrawWinnerRoundMessage'));

  for(const match of endText.matchAll(/"([^"\\]*)"/g)){
    assert(!CYRILLIC.test(match[1]),
      `2b: в конце матча по-русски: «${match[1]}» — рядом с ним английские строки`);
  }
  assert(/No one survived\./.test(endText) && /draw/i.test(endText),
    '2c: тексты конца матча прочитаны — и «никто не выжил», и ничья');
}
