...
// === 7g. Самолётики зазывают нажать «Send link» ===
//
// «Send link» встал ровно в слот Hot Seat — тот же y. Значит при выборе онлайна самолётики
// не сдвигаются ни на точку, и выбор режима остаётся вообще без отклика: со стороны это
// выглядит как «ничего не произошло». Вместо перелёта, которому некуда лететь, они тихо
// сходятся к надписи и расходятся.
//
// Ломается это тремя способами, и все тихие: зазывание не гаснет и мозолит глаза вечно;
// оно качает transform и дерётся с местом самолётика в кадре; оно доживает до экрана
// гостя, где нажимать нечего.
{
  const зазыв = /function обновитьЗазывание\(\)\{[\s\S]*?\n\}/.exec(source);
  assert(зазыв, '7g: расчёт зазывания не найден');

  // Гаснуть оно обязано по трём поводам сразу.
  for(const [условие, зачем] of [
    ['isOnlineHostSeat()', 'у гостя нажимать нечего — «Send link» ему не показывают'],
    ['!isOnlineTableFull()', 'соперник пришёл, и внимание переходит к загоревшемуся «Play»'],
    ['!onlineInviteSent', 'друга уже позвали, дело сделано — дальше только ждать'],
    ['onlineLobbyFocusBtn === onlineSendLinkBtn', 'выбор ушёл с кнопки, звать больше не на что'],
  ]){
    assert(зазыв[0].includes(условие),
      `7g1: зазывание не гаснет, когда ${зачем} (потерялось «${условие}»)`);
  }

  // Отметка «позвали» ставится в обеих ветках отправки — и через системную шторку, и
  // через буфер обмена. А вот отказ от шторки за отправку не считается: человек передумал,
  // и подсказка ему ещё пригодится.
  const блокОтправки = source.slice(
    source.indexOf('if(onlineSendLinkBtn instanceof HTMLElement){'),
    source.indexOf('if(onlineCancelBtn instanceof HTMLElement){'));
  const отметок = блокОтправки.split('onlineInviteSent = true').length - 1;
  assert(отметок === 2,
    `7g2: «позвали» отмечается ${отметок} раз вместо двух — одна из веток отправки `
    + '(шторка или буфер) оставит зазывание навсегда');

  // Тело первого catch — это отказ от шторки. Оно короткое и без вложенных скобок,
  // поэтому кончается на первой же закрывающей.
  const началоОтказа = блокОтправки.indexOf('catch(_error){');
  const телоОтказа = блокОтправки.slice(началоОтказа,
    блокОтправки.indexOf('}', началоОтказа));
  assert(!/onlineInviteSent = true/.test(телоОтказа),
    '7g3: закрытую шторку засчитали за отправку — человек передумал, а подсказка пропала');

  // Качается translate, а не transform: transform занят местом самолётика в кадре, и на
  // нём же висит переход перелёта.
  const кадры = /@keyframes mm-plane-beckon\s*\{([^}]*\}[^}]*)\}/.exec(styles);
  assert(кадры, '7g4: нет ключевых кадров зазывания');
  assert(!/transform/.test(кадры[1]),
    '7g5: зазывание качает transform — оно затрёт место самолётика в кадре и перелёт');
  assert(/translate/.test(кадры[1]), '7g6: зазывание ничего не двигает');

  assert(/\.is-online-alone[^{]*\.mm-plane--mode\s*\{[^}]*animation:\s*mm-plane-beckon/
    .test(styles), '7g7: зазывание не привязано к состоянию «хозяин один»');
  assert(/prefers-reduced-motion[\s\S]{0,400}is-online-alone[^{]*\{[^}]*animation:\s*none/
    .test(styles), '7g8: зазывание не гасится в режиме без движения');
}

// === 7h. Отмена: перелёт успевает приземлиться ===
//
// Самолётики летят к «Cancel», и только потом игра уходит из комнаты. Пауза была ровно
// в длину перелёта — и замер показывал разворот на полпути: выход срабатывал в тот же миг,
// когда перелёт заканчивался, и дёргал их обратно. Числа связаны, и связь эта видна только
// глазами на живом экране.
{
  const перелёт = /const ONLINE_PLANE_FLIGHT_MS = (\d+);/.exec(source);
  assert(перелёт, '7h: длина перелёта не названа числом');

  const пауза = /\}, ONLINE_PLANE_FLIGHT_MS \+ (\d+)\);/.exec(source);
  assert(пауза, '7h1: пауза перед выходом больше не считается от длины перелёта — '
    + 'разъедутся, и перелёт снова оборвёт на полпути');
  assert(Number(пауза[1]) >= 30,
    `7h2: пауза длиннее перелёта всего на ${пауза[1]} мс — этого мало, чтобы приземлиться`);

  // Возврат к Hot Seat — склейкой, а не вторым перелётом подряд.
  assert(/classList\?\.add\?\.\("is-cut"\)|classList\.add\("is-cut"\)/.test(source),
    '7h3: возврат самолётиков больше не склеивается — на фоне смены экрана второй перелёт '
    + 'подряд читается рывком');
  assert(/\.mm-plane\.is-cut\s*\{[^}]*transition:\s*none/.test(styles),
    '7h4: у склейки нет стиля — она ничего не отключает');
}
