...
// === 15. «Подключаемся» — не «связь потеряна» ===
//
// Разница не косметическая. Экран лобби показывается сразу, как позвали комнату, а сокет
// к этому мигу ещё открывается. Одно состояние на оба случая — и человек, впервые нажавший
// «Online», первым делом читает, что связь потеряна.
//
// Раздел приехал из smoke-online-join-code: вход по коду убрали вместе с обоснованием, а
// это к кодам никогда и не относилось — это про провод.
{
  ...
  assert(/everOpened \? "reconnecting" : "connecting"/.test(transport),
    '15: до первого открытия состояние — «connecting», а не «reconnecting»');
  assert(/everOpened = true;/.test(transport),
    '15b: и «открывались» отмечается при открытии');

  // Про поломки говорят только когда они есть. Пока сокет открывается — молчат: терять
  // ещё нечего, пугать нечем.
  const trouble = extractFunctionSource(source, 'getOnlineTroubleText');
  assert(/connection !== "connecting"/.test(trouble),
    '15c: «подключаемся» больше не отличается от обрыва — игрок увидит «связь потеряна» '
  ...
  // навсегда застынет то, что было в миг открытия лобби.
  assert(/createWebSocketTransport\(room, seatInfo\.relay, seatInfo\.seat, refreshOnlineLobbyUi,/
    .test(source), '15d: смена состояния связи перерисовывает лобби');
  const announces = (transport.match(/announce\(\);/g) || []).length;
  assert(announces >= 3,
    `15e: сообщается и об открытии, и об обрыве, и об отказе (нашлось ${announces})`);
}
