...
const build = ({ gameMode, turnColor }) => {
  const sandbox = {
    gameMode,
  ...
  vm.runInContext([
    ...
    'this.api = { getColorController, isLocalColor, isAiColor, isAiControlledTurn, AI_PLAYER_COLOR };',
  ].join('\n'), sandbox);
  return sandbox.api;
};
...
// === 4. Понятие годится для онлайна, а не только для ИИ ===
//
// «Не моя сторона» и «сторона ИИ» — разные вещи: удалённого игрока не надо запускать,
// его ход приедет сам. Поэтому предикатов два, и они не должны быть одним и тем же.
{
  const controllers = source.match(/const COLOR_CONTROLLERS = Object\.freeze\(\{([^}]*)\}\);/)[1];
  assert(/LOCAL:/.test(controllers) && /AI:/.test(controllers),
    '4: у стороны есть как минимум «своя» и «ИИ»');

  const localFn = extractFunctionSource(source, 'isLocalColor');
  const aiFn = extractFunctionSource(source, 'isAiColor');
  assert(/COLOR_CONTROLLERS\.LOCAL/.test(localFn) && /COLOR_CONTROLLERS\.AI/.test(aiFn),
...
// Иначе онлайн придётся чинить в шести местах вместо одного.
{
  const strays = source
    ...
    .filter(({ line }) => /gameMode === "computer"/.test(line))
...
console.log('Smoke test passed: «кто управляет стороной» собрано в одном месте, решает ровно то же, что прежние шесть условий, и различает ИИ и просто чужую сторону.');
