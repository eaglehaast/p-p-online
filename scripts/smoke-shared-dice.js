...
const ALLOWED = {
  ...
  makeRandomString: 'общий генератор имён комнат и ключей мест: и то и другое обязано быть '
    + 'непредсказуемым, иначе к чужой партии подсядет посторонний. Общий жребий тут не при '
    + 'чём — комнаты ещё нет. Берёт crypto.getRandomValues, а Math.random() держит запасной '
    + 'веткой для очень старых браузеров, где игра всё равно должна открыться',
