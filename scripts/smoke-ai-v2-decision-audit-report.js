...
const isActualTacticalItemConsumptionEventSrc = extractFunctionSource(source, 'isActualTacticalItemConsumptionEvent');
...
const context = {
  ...
  exportAiSelfAnalyzerTurnsJson: () => ({
    ...
    source: {
      events: [
        { type: 'ai_decision', stage: 'v2_shot_plan_not_found', reasonCodes: ['v2_shot_plan_not_found', 'mine_route_probe_center'], selectedMove: { decisionReason: 'mine_route_probe_center' } },
      ],
    },
  }),
...
assert(report.summary.stableMetrics.matches_with_mine_or_dynamite_share === 0, 'Reason codes alone must not count as mine/dynamite usage.');
...
assert(compactReport.stableMetrics.matches_with_mine_or_dynamite_share === 0, 'Compact report must keep mine/dynamite share at 0 without real consumption events.');
