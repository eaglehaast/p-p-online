...
// Отражена ровно одна из двух морд: пара для горизонтали уложена на лист в одну
// сторону, а углы у морд противоположные. Какая именно и почему — разбирает
// smoke-mascot-sheet; здесь важно только, что отражение не размножилось и не
// пропало. Тело правила берём до ближайшей закрывающей скобки: с [\s\S]*? проверка
// протекала в следующее правило и держалась на чужом scaleX.
assert(!/html\.is-board-landscape #mantisIndicator\s*\{[^}]*scaleX\(-1\)/.test(styles),
  '6g: козла в горизонтали снова отражают — его голова на листе уже смотрит влево');
assert(/html\.is-board-landscape #goatIndicator\s*\{[^}]*scaleX\(-1\)/.test(styles),
  '6g2: воробей перестал отражаться — в левом нижнем углу он смотрит наружу поля');
...
// а копыто оставалось на прежнем месте — вылезало сверху, боком и в стороне от него.
{
  ...
  assert(/rotate\(-90deg\)/.test(goatLand),
    '12b: козёл в горизонтали развёрнут встречным поворотом — копыто должно повторять это');
  // Копыто отражено (--hoof-flip: -1), а в правиле козла отражения нет — и это не
  // рассогласование. Копыто повторяет козла НА ЭКРАНЕ, а там он смотрит влево; раньше
  // это давал scaleX над портретной головой, теперь голова уже нарисована смотрящей
  // влево (правая колонка листа, см. smoke-mascot-sheet). Экранная сторона не
  // изменилась, поэтому копыто осталось прежним.
  assert(/--hoof-flip:\s*-1/.test(land),
    '12b2: копыто перестало отражаться — оно потянется не в ту сторону от морды');
  const goatFromU = 0;
  const goatToU = 104;
