...
// Отражена ровно одна из двух морд — та, что стоит в углу кадра (0,0): на экране это
// правый верхний угол, и «внутрь поля» там влево, тогда как все головы на листе
// нарисованы смотрящими вправо. Вся матрица «край на ориентацию на морду» разбирается
// в smoke-mascot-sheet; здесь важно только, что отражение не размножилось и не
// пропало. Тело правила берём до ближайшей закрывающей скобки: с [\s\S]*? проверка
// протекала в следующее правило и держалась на чужом множителе.
assert(/html\.is-board-landscape #mantisIndicator\s*\{[^}]*--corner-flip:\s*-1/.test(styles),
  '6g: козёл в горизонтали стоит в углу кадра (0,0) и обязан отражаться — иначе смотрит '
  + 'наружу поля');
assert(!/html\.is-board-landscape #goatIndicator\s*\{[^}]*--corner-flip:\s*-1/.test(styles),
  '6g2: воробья в горизонтали отражают, хотя он стоит в противоположном углу — там его '
  + 'голова и так смотрит внутрь поля');
...
// а копыто оставалось на прежнем месте — вылезало сверху, боком и в стороне от него.
{
  ...
  assert(/--turn:\s*-90deg/.test(goatLand),
    '12b: козёл в горизонтали развёрнут встречным поворотом — копыто должно повторять это');
  assert(/--corner-flip:\s*-1/.test(goatLand) && /--hoof-flip:\s*-1/.test(land),
    '12b2: козёл и копыто отражаются по-разному — копыто потянется не в ту сторону от морды');
  // Считано это только для ОБЫЧНОГО края: при своём крае снизу козёл уезжает в
  // противоположный угол, а копыто остаётся здесь. Так решено сознательно — анимацию
  // «ИИ думает» ещё будут менять, и подпирать её сейчас нечем: копыто козье, а ИИ при
  // перевороте играет воробьём.
  const goatFromU = 0;
  const goatToU = 104;
