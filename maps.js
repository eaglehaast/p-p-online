...
// Карты читаются как обычные файлы — с кэшем.
//
// Раньше здесь стояло { cache: 'no-store' }: манифест и все сорок одну карту браузер
// качал заново при КАЖДОМ запуске игры, мимо кэша, всегда. Замер на 600 кбит/с с
// задержкой 300 мс, повторный заход:
//
//   было:  42 запроса, 360 КБ, карты готовы через 18.6 с
//   стало: 42 ответа из кэша, 0 КБ, карты готовы через 0.17 с
//
// Опасение, ради которого no-store обычно и ставят, — что игрок останется на старой
// карте после обновления, а в онлайне это разошлось бы с соперником. Здесь оно не
// работает: script.js, стили и все картинки кэшируются ровно так же. Игрок со старым
// кэшем держит старым ВСЁ, согласованно; исключение для одних только карт не защищало
// бы от расхождения, а наоборот делало бы карты рассинхронизированными с кодом.
//
// Редактору карт это тоже не мешает: worker/local-server.js отдаёт всё с cache-control:
// no-cache, поэтому на своём компьютере ничего не кэшируется и правка видна сразу.
